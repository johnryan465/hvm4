...
#define UNS                                                                    \
  40 // Unscoped(xf, xv): binds an unscoped lambda/var pair to xf and xv
#define ANY 41 // Any: wildcard that duplicates itself and equals anything
#define INC                                                                    \
  42 // Inc(x): priority wrapper for collapse ordering - decreases priority
...
// Note: regular term tags (APP, MAT, USE, DP0, DP1, OP2, DSU, DDU) also used as
// frames These frames reuse existing heap nodes to avoid allocation
#define F_APP_RED                                                              \
  0x40 // ((f ~> □) x): val=app_loc. RED at HEAP[app_loc], arg at
       // HEAP[app_loc+1]
#define F_RED_MAT                                                              \
  0x41 // ((f ~> mat) □): val=app_loc. After reducing g, mat stored at
       // HEAP[red_loc+1]
#define F_RED_USE                                                              \
  0x42 // ((f ~> use) □): val=app_loc. After reducing g, use stored at
       // HEAP[red_loc+1]
...
static u64 HEAP_NEXT[MAX_THREADS * HEAP_STRIDE]
    __attribute__((aligned(256))) = {0};
static u64 HEAP_END[MAX_THREADS * HEAP_STRIDE]
    __attribute__((aligned(256))) = {0};
...
static const char *nick_alphabet =
...
static u32 PARSE_FRESH_LAB =
    0x800000; // start at 2^23 to avoid collision with user labels
static int PARSE_FORK_SIDE =
    -1; // -1 = off, 0 = left branch (DP0), 1 = right branch (DP1)
...
#include "wnf/app_dup.c"
#include "wnf/app_era.c"
#include "wnf/app_got.c"

...
//   left/strict field (APP fun, OP2 lhs, etc). DP0/DP1 push and descend into
//   the shared dup expr. MAT/USE/RED add specialized frames when their
//   scrutinee is ready.
...
enter : {
  ...
  case ALO: {
    ...
    case BJM: {
      next = wnf_alo_got(ls_loc, len, term_val(book), 0);
      goto enter;
    }
    case BJ0:
    case BJ1: {
    ...
    case DDU:
    case RED: {
      next = wnf_alo_nod(ls_loc, len, term_val(book), term_tag(book),
                         term_ext(book), term_arity(book));
      goto enter;
    }
    case DUP: {
    ...
    case MOV: {
      next = wnf_alo_mov(ls_loc, len, term_val(book), 0);
      goto enter;
    }
    case NUM: {
...
apply : {
  static _Thread_local u64 trace_count = 0;
  if (trace_count < 1000 && __builtin_expect(DEBUG, 0)) {
    printf("wnf_apply: ");
    print_term(whnf);
    printf("\n");
    trace_count++;
  ...
  while (s_pos > base) {
    ...
    case APP: {
      ...
      case DP0:
      case DP1: {
        whnf = wnf_app_dup(whnf, arg);
        continue;
      }
      case GOT: {
        whnf = wnf_app_got(whnf, arg);
        continue;
      }
      case SUP: {
    ...
    case GOT: {
      ...
      switch (term_tag(whnf)) {
      case GOT: {
        whnf = wnf_mov_mov(loc, whnf, lab, side);
        continue;
      }
      case NAM:
      case BJV:
      ...
      default: {
        u64 new_loc = heap_alloc(1);
        heap_set(new_loc, whnf);
        Term got = term_new_got(side, lab, (u32)new_loc);
    ...
    case F_EQL_R: {
      ...
      default: {
        ...
        // NAM/BJ* === NAM/BJ*
        if ((a_tag == NAM || a_tag == BJV || a_tag == BJM || a_tag == BJ0 ||
             a_tag == BJ1) &&
            (b_tag == NAM || b_tag == BJV || b_tag == BJM || b_tag == BJ0 ||
             b_tag == BJ1)) {
